...
function HTMLParser(address, fragmentContext, options) {
  ...
  // Reconstructing stale active formatting elements can otherwise turn a
  // small input into a quadratic number of DOM allocations. Keep the total
  // reconstruction work linear in the amount of input supplied to this
  // parser. The floor avoids affecting small documents with ordinary HTML
  // error recovery, while the per-character allowance scales for large
  // documents.
  var input_char_count = 0;
  var afe_reconstruction_count = 0;
  var MIN_AFE_RECONSTRUCTION_BUDGET = 4096;
  var AFE_RECONSTRUCTIONS_PER_INPUT_CHAR = 2;
  ...
  var htmlparser = {
    ...
    parse: function(s, end, shouldPauseFunc) {
      ...
      // Count only newly supplied input. Buffered leftovers are already
      // accounted for, while process() and resume() re-enter with an empty
      // string and therefore do not count the same input twice.
      input_char_count += s.length;
  ...
  function afereconstruct() {
    ...
    for(i = i+1; i < afe.list.length; i++) {
      afe_reconstruction_count++;
      var reconstruction_budget = Math.max(
        MIN_AFE_RECONSTRUCTION_BUDGET,
        input_char_count * AFE_RECONSTRUCTIONS_PER_INPUT_CHAR
      );
      if (afe_reconstruction_count > reconstruction_budget) {
        throw new Error('HTML parser active formatting reconstruction limit exceeded');
      }
